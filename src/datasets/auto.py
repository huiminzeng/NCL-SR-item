...
class AutoDataset(AbstractDataset):
    @classmethod
    def code(cls):
        return 'auto'
    ...
    def url(cls):
        ...
        return ['http://snap.stanford.edu/data/amazon/productGraph/categoryFiles/ratings_Automotive.csv',
    ...
    def all_raw_file_names(cls):
        return ['auto.csv', 'auto_meta.json.gz']
        ...
        with gzip.open(file_path, 'rb') as f:
            for line in f:
                ...
                if 'title' in item and len(item['title']) > 0:
                    ...
                    if len(item['categories'][0]) >= 4:
                        cate = item['categories'][0][:4]
                    else:
